...
var Collide = function() {
    ...
    function trianglePoint(ta, dot) {
        ...
        var alpha = ((ta.p2.y - ta.p3.y) *
            (dot.x - ta.p3.x) +
            (ta.p3.x - ta.p2.x) * (dot.y - ta.p3.y)) /
            ((ta.p2.y - ta.p3.y) *
                (ta.p1.x - ta.p3.x) +
                (ta.p3.x - ta.p2.x) * (ta.p1.y - ta.p3.y));

        var beta = ((ta.p3.y - ta.p1.y) *
            (dot.x - ta.p3.x) + (ta.p1.x - ta.p3.x) * (dot.y - ta.p3.y)) /
            ((ta.p2.y - ta.p3.y) *
                (ta.p1.x - ta.p3.x) +
                (ta.p3.x - ta.p2.x) *
                    (ta.p1.y - ta.p3.y));
    ...
    function circlePoint(c, p) {
        ...
        var distR = Math.sqrt(
            Math.pow(p.x - c.pos.x, 2) +
                Math.pow(p.y - c.pos.y, 2));
